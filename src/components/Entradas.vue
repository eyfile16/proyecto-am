<template>
  <q-page padding>
    <div class="text-h4 text-primary text-center q-mb-lg">Gestión de Inventario - Entradas</div>
    ...
    <q-dialog v-model="showModal" persistent maximized>
      <q-card class="column" style="width: 900px; max-width: 95vw;">
        <q-card-section class="bg-primary text-white">
          <div class="text-h6">{{ isEditMode ? 'Editar Entrada' : 'Nueva Entrada' }}</div>
        ...
        <q-td :props="props" class="text-center">
          <q-btn-group flat>
            <q-btn
              ...
              size="sm"
            >
              <q-tooltip>Editar entrada</q-tooltip>
            </q-btn>
            <q-btn
...
async function loadEntries() {
  try {
    isLoading.value = true;
    const response = await getData('/movimientos/tipo/1');
    
    if (response && response.entradas) {

      inventoryEntries.value = [...response.entradas];
    }
  } catch (error) {
...
async function submitEntry() {
  try {
    ...
    const errorMessage = error.response?.data?.error || 'Error al guardar la entrada';
